...
Function Resolve-Tenat-IDs{
    <#
    .SYNOPSIS

        A function to resolve tenant ids to domain names
    ...
    $tenant_ids = @(Get-Content -Path $tenantlist)

    if (!$tenant_ids){return}

    $results = @()

    foreach($id in $tenant_ids){

        try {$request = Invoke-WebRequest -Method GET -Uri "https://graph.microsoft.com/v1.0/tenantRelationships/findTenantInformationByTenantId(tenantId='$id')" -Headers @{"Authorization" = "Bearer $access_token"}
        }catch{
            $err = $_.Exception.Response.StatusCode.Value_
        }
        ...
        If ($request.StatusCode -eq 200){
                ...
                $record = @{
                    ...
                    "domain_name" = $out.defaultDomainName
                }
    ...
    if (!$out_file){
        ...
        $results | Export-Csv -NoTypeInformation -Append $out_file
    }
}

if (!$tokens){Get-GraphTokens}

Resolve-Tenat-IDs -Tokens $tokens -tenantlist ".\ids.txt"